...
int galaga(void)
{
	...
	while (1)
	{
		...
		for (int a = 0; a < 9; a++)
		{
			...
			if (!easyEnemies[a].collisionFlag && collision(easyEnemies[a].enemyX, easyEnemies[a].enemyY, 20, 20, player.playerX, player.playerY))
			{
				send(pidVida, -1);				  // Enviar el mensaje solo una vez si no se ha enviado antes
				easyEnemies[a].collisionFlag = 1; // Establecer la bandera de colision
				//endGame();
			}
			if (easyEnemies[a].enemyY >= 160)
			{
				easyEnemies[a].enemyY = 0;
				easyEnemies[a].collisionFlag = 0; // Reiniciar la bandera de colision al reiniciar la posicion del enemigo
			}
		}
...
for (int a = 0; a < 9; a++)
{
    ...
    if (!hardEnemies[a].collisionFlag && collision(hardEnemies[a].enemyX, hardEnemies[a].enemyY, 20, 20, player.playerX, player.playerY))
    {
        send(pidVida, -1); 
        hardEnemies[a].collisionFlag = 1; 
    }
    ...
    {
        endGame();
        break; // Salir del bucle para evitar ejecuciones múltiples de endGame()
    }
}
...
void endGame()
{
	...
	// Enviar mensaje al proceso vida
	// send(pidVida, -1);

	// Comprobar si las vidas llegaron a cero
	if (vidas <= 0)
	{
